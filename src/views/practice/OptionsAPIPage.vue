...
export default {
  ...
  watch: {
    list: {
      handler(newVal) {
        console.log('watch监听到list数组发生变化', newVal)
      },
      deep: true
    }
  },
  methods: {
   ...
   handleDel(id){
    ...
    toggleDone(id) {
      const target = this.todos.find(i => i.id === id)
      if (target) target.done = !target.done
    },
   
    // 计数器
    addNum() { if (this.count < 20) this.count++ },
    delNum() { if (this.count > 0) this.count-- },
    resetNum() { this.count = 5 },
